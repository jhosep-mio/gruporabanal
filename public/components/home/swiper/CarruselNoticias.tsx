...
export const CarruselNoticias = ({ data, id }: any) => {
    ...
    const handleSwiper = () => {
      ...
      if (swiperInstance) {
        const prevButton = document.querySelector(
          `.custom-swiper-button-prev${id}`
        );
        const nextButton = document.querySelector(
          `.custom-swiper-button-next${id}`
        );
  ...
  return (
    <div className="relative w-full mt-4 lg:mt-6">
      ...
      >
        {data?.map((product: any) => (
          <SwiperSlide className="" key={product.id}>
            <div
              onClick={() => router.push(`/noticias/${product.url}`)}
            ...
            >
              ...
              <div className="p-4">
                <h3 className="text-lg font-montserratBold line-clamp-1">
                  {product.titulo}
                </h3>
                <h4 className="text-xs text-gray-500 mb-2 ">Noticia</h4>
                <div className="text-gray-600 text-sm line-clamp-3 mb-4">
                  {product.contenido}
                </div>
                <Link
                  href={`/collections/${product.url_categoria}/${product.url}`}
                  className=" bg-primary hover:bg-secondary transition-all rounded-md px-2 lg:px-6 py-2 w-full lg:w-fit block text-center text-white font-proximaSemibold text-sm lg:text-lg"
                >
                  Leer más
                </Link>
              </div>
      ...
      <div
        className={`grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-1 mx-auto ${
