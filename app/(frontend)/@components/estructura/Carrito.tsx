...
import { calculateTotal, decreaseItemQuantity, increaseItemQuantity } from '@/components/Funtions'

export const Carrito = ({ openCar, setOpenCar }: any) => {
  const { cart, setCart } = useAuthPublic()
            ...
            cart.map((item: any, index: number) => (
              <div key={index} className="flex items-center gap-4 justify-between group py-4 ">
                <div className="flex items-center gap-4 h-[100px] w-full">
                  <img src={`${process.env.NEXT_PUBLIC_URL}/productos/uploads/${item.imagen1}`} alt="" className="w-[80px] lg:w-[100px] h-[100px] object-contain" />
