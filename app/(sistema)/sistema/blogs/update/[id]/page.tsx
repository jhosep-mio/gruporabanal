...
export default function UpdateBlogPage () {
  ...
  const updateBlog = async (): Promise<void> => {
    ...
    setLoading(true)
    try {
      const { data } = await axios.post(`${process.env.NEXT_PUBLIC_BASE_URL}/api/blogs/update/${id}`, formData, {
        headers: {
  ...
  useEffect(() => {
    const getBlog = async (): Promise<void> => {
      try {
        const { data } = await axios.get(`${process.env.NEXT_PUBLIC_BASE_URL}/api/blogs/find/${id}`)
        setValues({
          titulo: data?.blog?.titulo,
          resumen: data?.blog?.resumen
        })
        setImagen1(data?.blog?.imagen1)
        setDescripcion(data?.blog?.descripcion ? JSON.parse(data?.blog?.descripcion) : '')
      } catch (error) {
        console.log(error)
      } finally {
        setLoading(false)
      ...
      <div className="rounded-xl border bg-card text-card-foreground shadow">
        <div className="flex flex-col space-y-1.5 p-6">
          <div className="flex items-center justify-between">
            <div className="font-semibold tracking-tight text-xl">Editar blog</div>
          </div>
        </div>
