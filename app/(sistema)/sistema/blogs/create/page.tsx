...
export default function CreateBlogPage () {
  ...
  const saveCategoria = async (): Promise<void> => {
    ...
    setLoading(true)
    try {
      const { data } = await axios.post(`${process.env.NEXT_PUBLIC_BASE_URL}/api/blogs/register`, formData, {
        headers: {
      ...
      if (data.status == true) {
        ...
        toast({
          duration: 3000,
          title: 'Error al registrar el blogs',
          variant: 'destructive'
        })
      }
    } catch (error: any) {
      ...
      if (dataError) {
        ...
        toast({
          duration: 3000,
          title: 'Error al registrar el blog',
          variant: 'destructive'
        })
      }
    } finally {
      ...
      <div className="rounded-xl border bg-card text-card-foreground shadow">
        <div className="flex flex-col space-y-1.5 p-6">
          <div className="flex items-center justify-between">
            <div className="font-semibold tracking-tight text-xl">Crear blog</div>
          </div>
        </div>
