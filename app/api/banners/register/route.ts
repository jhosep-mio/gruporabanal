...
export async function POST (req: NextRequest) {
  ...
  try {
    ...
    const uploadDir = path.join(process.cwd(), 'private/uploads', 'banners') // Carpeta privada
    ...
    if (imagen2) await processImage(imagen2, 'imagen2')
    const formattedName1 = savedImages.imagen1 || null

    // Guardar en la base de datos si es necesario
    await BannersModel.create({ imagen: formattedName1, titulo1, titulo2, texto })
